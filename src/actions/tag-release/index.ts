...
export async function runTagRelease(): Promise<void> {
  ...
  const args = ['step', 'tag-release', '--from', ref, '--format', 'json'];
  ...
  const tags = isRecord(parsed?.tags) ? parsed.tags : undefined;
  core.setOutput('result', 'success');
  core.setOutput('json', JSON.stringify(parsed ?? null));
  core.setOutput('tags', JSON.stringify(tags ?? {}));
  core.setOutput('tag', id ? outputValue(tags?.[id]) : '');
}
